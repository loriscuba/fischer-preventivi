...
def parse_righe(testo, numero_ordine):
    righe = []
    pattern = re.compile(
        r"(\d{8})\s+"
        r"(.+?)\s+"
...
def upsert(tabella, record):
    ...
    if r.status_code not in (200, 201):
        ...
        print(f"OK {tabella}: {record.get('numero_ordine') or record.get('cod_art')}")
...
def main():
    ...
    for nome_file, pdf_bytes in allegati:
        ...
        if not numero_ordine:
            print(f"Numero ordine non trovato in {nome_file}, skip.")
            continue
